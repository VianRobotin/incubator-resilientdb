...
Tusk::Tusk(int id, int f, int total_num, SignatureVerifier* verifier,
          ...
          std::function<void(const Proposal* p)> collect_timestamp)
      : id_(id), f_(f), verifier_(verifier), total_num_(total_num), 
      single_call_(single_call), broadcast_call_(broadcast_call), commit_(commit), collect_timestamp_(collect_timestamp){
  ...
  process_execute_thread_ = std::thread(&Tusk::AsyncProcessCausalHistory, this);
...
// [DK] AysncSend ====> CommitRound ====> AsyncCommitLeaderVertex ====> CommitLeaderVertex ====> AsyncCommitCausalHistory ====> AsyncProcessCausalHistory 
// ====> FairDAG::CommitTxns ====>
...
void Tusk::AsyncSend() {
  ...
  while (!IsStop()) {
    ...
    // LOG(ERROR)<<"gen proposal block at round " << proposal_manager_->CurrentRound();
    ...
    //[DK] Commit the Blocks in the round-2
    if (round > 1) {
      if (round % 2 == 0) {
        CommitRound(round - 2);
      } 
  ...
  while (!IsStop()) {
    ...
    for (int r = last_committed_round + 2; r <= round; r += 2) {
      ...
      if (reference_num < limit_count_) {
        ...
        for (int j = last_committed_round+2; j <= r; j += 2) {
      ...
      last_committed_round = r;
    }
  }
}

void Tusk::AsyncProcessCausalHistory() {
  while (!IsStop()) {
    auto txns_w_ts = commit_txn_queue_.Pop();
    if(txns_w_ts == nullptr) {
      continue;
    }

    /*
    int64_t in_time = (*txns)[0]->queuing_time();
    (*txns)[0]->set_queuing_time(0);
    int64_t wait_time = GetCurrentTime() - in_time;
    int64_t commit_time = GetCurrentTime(); 
 
    global_stats_->AddCommitQueuingLatency(wait_time);
    */
    commit_(*txns_w_ts);
  }
}
...
std::unique_ptr<Transaction> Tusk::FetchTxnCopy(const std::string& hash){
  return proposal_manager_->FetchTxnCopy(hash);
}
...
void Tusk::AsyncCommitCausalHistory() {
  while (!IsStop()) {
    ...
    std::map<int, std::vector<std::unique_ptr<Proposal>>> vertices;
    std::queue<std::unique_ptr<Proposal>>q;
    q.push(std::move(proposal));

    // [DK] Get the causal history of a committed leader vertex
    while(!q.empty()){
      ...
      vertices[p->header().round()].push_back(std::move(p));
    }


    // [DK] Gather the digest and timestamp of each transaction in the causal history of the a committed leader vertex
    int num = 0;
    int pro = 0;
    std::unique_ptr<std::vector<std::unique_ptr<Transaction> >> txns_w_ts = std::make_unique<std::vector<std::unique_ptr<Transaction> >>();
    ...
    for(auto& it: vertices){
      for(auto& p : it.second){
        ...
        for(TxnDigest& dg : *p->mutable_digest()){
          ...
          //LOG(ERROR)<<"commit from:"<<dg.proposer()<<" user seq:"<<dg.seq()<<" ts:"<<dg.ts()<<" txn proposer:"<<txn->proposer()<<" delay:"<<(GetCurrentTime()- txn->create_time() - txn->queuing_time())<<" queuing time:"<<txn->queuing_time()<<" proposer create time:"<<p->header().create_time()<<" txn create time:"<<txn->create_time() <<" queuing time:"<<txn->queuing_time();
          txns_w_ts->push_back(std::move(txn));
          num++;
        }
        pro++;
      }
    }
    // LOG(ERROR)<<"commit proposal:"<<txns_w_ts.size()<<" round:"<<commit_round<<" blocks:"<<pro<<" txn size:"<< num<<" uniq:"<<v.size();
    //commit_(*txns_w_ts);
    (*txns_w_ts)[0]->set_queuing_time(GetCurrentTime());
    commit_txn_queue_.Push(std::move(txns_w_ts));
    ...
    //LOG(ERROR)<<" commit blocks:"<<blocks;
  }
}
...
void Tusk::AsyncProcessCert(){
  while(!IsStop()){
    ...
    collect_timestamp_(proposal_manager_->GetRequest(round, cert_sender));
