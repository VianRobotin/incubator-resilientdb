...
int PerformanceManager::BatchProposeMsg() {
  const int sender_threads = 2;
  const uint32_t target_tps = config_.GetTargetInputTPS();
  const bool rate_limit = target_tps > 0;
  std::chrono::steady_clock::time_point next_send_time;
  std::chrono::steady_clock::duration send_interval(0);
  if (rate_limit) {
    double per_thread_tps =
        static_cast<double>(target_tps) / sender_threads;
    send_interval = std::chrono::duration_cast<std::chrono::steady_clock::duration>(
        std::chrono::duration<double>(1.0 / per_thread_tps));
    next_send_time = std::chrono::steady_clock::now();
  }
  LOG(WARNING) << "batch wait time(us):" << config_.ClientBatchWaitTimeUS()
  ...
  while (!stop_) {
    ...
    if (batch_req.size() < config_.ClientBatchNum()) {
      std::unique_ptr<QueueItem> item =
          batch_queue_.Pop(config_.ClientBatchWaitTimeUS());
      if (item == nullptr) {
        continue;
      }
