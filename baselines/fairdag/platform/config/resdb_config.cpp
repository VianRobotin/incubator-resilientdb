...
uint32_t ResDBConfig::ClientBatchWaitTimeUS() const {
  if (config_data_.client_batch_wait_time_us()) {
    return config_data_.client_batch_wait_time_us();
  }
  return client_batch_wait_time_us_;
}

void ResDBConfig::SetClientBatchWaitTimeUS(uint32_t wait_time_us) {
  config_data_.set_client_batch_wait_time_us(wait_time_us);
  client_batch_wait_time_us_ = wait_time_us;
}
