...
USERNAME="${BASELINES_USER:-vrobotin}"
BASE_PORT=17000
...
bazel build //tools:certificate_tools > /dev/null
bazel build //tools:generate_region_config > /dev/null
...
do
  ...
  if [ $(($idx+$CLIENT_NUM)) -gt $tot ] ; then
    $CERT_TOOLS_BIN ${output_cert_path} ${ADMIN_PRIVATE_KEY} ${ADMIN_PUBLIC_KEY} ${public_key} ${idx} ${ip} ${port} client > /dev/null
    echo "${idx} ${ip} ${port}" >> client.config
    echo "${idx} ${ip} ${port}" >> client${client_idx}.config
    client_idx=$((client_idx+1))
  else
    $CERT_TOOLS_BIN ${output_cert_path} ${ADMIN_PRIVATE_KEY} ${ADMIN_PUBLIC_KEY} ${public_key} ${idx} ${ip} ${port} replica > /dev/null
