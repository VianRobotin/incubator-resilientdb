#!/usr/bin/env python3
# CLI wrapper around das.run() for the batch-size sweep orchestrator.
# Runs ONE point: (n, batch, rate, rl) and prints a single-line JSON
# result on stdout.  Stderr carries the underlying script's chatter.
#
# Usage (from baselines-fairdag/scripts/deploy):
#   python3 performance/das_batch.py --n 16 --batch 100 --rate 500 --rl 0
#   python3 performance/das_batch.py --n 16 --batch 100 --rate 500 --rl 1
...
def main():
    ...
    p.add_argument("--batch", type=int, required=True,
                   help="block size (number of txns per block; sets FAIRDAG_BLOCK_SIZE)")
