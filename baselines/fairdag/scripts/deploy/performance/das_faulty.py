#!/usr/bin/env python3
# CLI wrapper around das.run() for the faulty-node sweep orchestrator.
# Runs ONE point: (n, faults, rate, rl) and prints a single-line JSON
# result on stdout.  Stderr carries the underlying script's chatter.
#
# Usage (from baselines-fairdag/scripts/deploy):
#   python3 performance/das_faulty.py --n 16 --faults 1 --rate 1000 --rl 0
#   python3 performance/das_faulty.py --n 16 --faults 3 --rate 1000 --rl 1
...
def main():
    ...
    p.add_argument("--faults", type=int, default=0, help="silent replicas to drop")
