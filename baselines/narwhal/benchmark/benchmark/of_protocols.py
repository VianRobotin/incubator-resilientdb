...
class OFBench:
    ...
    def _kill_nodes(self):
        ...
        try:
            if not self.local:
                ...
                # Pompe flushes elapsed_exec to disk only after SIGTERM unwinds the
                # event loop; large vectors at high rates take >5s to write.
                sleep(20 if is_pompe_flavor else 5)
    ...
    def _silence_last_f_replicas(self, num_faults, replica_IPs):
        """Silent-crash the last `num_faults` replicas after they come up.

        Mirrors FairDAG's deploy.sh:171-190 pattern. Neither pompe-app nor
        the dikaios hotstuff-app (themis) honor the `--num-faults` cmdline
        flag, so the only way to actually inject silent faults at this layer
        is to hard-kill the trailing F replica processes once they have
        bound their listeners. Clients then see established peers drop —
        the standard BFT silent-crash failure model.

        Match each replica by its per-index config file
        (`hotstuff-sec{i}.conf`) which is unique in the cmdline for both
        flavors, so we kill exactly the intended replica.
        """
        if num_faults <= 0:
            return
        n = self.nodes[0]
        ...
        if live <= 0:
            Print.warn(
                f"silence: num_faults={num_faults} leaves {live} live "
                f"replicas (n={n}); skipping"
            )
            return
        Print.info(
            f"FAULTS={num_faults}: silencing replicas {live}..{n-1} "
            f"(silent-crash) — {live} live"
        )
        for i in range(live, n):
            pattern = f"hotstuff-sec{i}\\.conf"
            if self.local:
                kill_cmd = f"pkill -9 -f '{pattern}' || true"
                subprocess.run(["bash", "-lc", kill_cmd],
                               stderr=subprocess.DEVNULL)
                Print.info(f"  silenced replica {i} on localhost")
            else:
                host = replica_IPs[i]
                ssh_cmd = (
                    f"ssh -o BatchMode=yes -o StrictHostKeyChecking=no "
                    f"{host} \"pkill -9 -f '{pattern}' || true\""
                )
                subprocess.run(ssh_cmd, shell=True,
                               stderr=subprocess.DEVNULL)
                Print.info(f"  silenced replica {i} on {host}")
    ...
    def _preserve_machines(self, nodes):
        ...
        # Reserve extra to absorb both BANNED_NODES landing in the reservation
        # and partial fulfillment from the scheduler.
        slack = 4
        request_count = self._num_machines + len(BANNED_NODES) + slack

        time_string = str(datetime.timedelta(seconds=self.duration + 72 + nodes * 2)) # extra time to set up things
        self.reservation_id = self.preserve_manager.create_reservation(request_count, time_string)

        # Wait until the reservation is Running with all machines assigned.
        # Without this, _get_hostnames() can return a partial list and downstream
        # code IndexErrors when packing clients onto missing machines.
        deadline = _now() + 60  # seconds
        last_state = None
        last_assigned = -1
        while _now() < deadline:
            try:
                res = self.preserve_manager.fetch_reservation(str(self.reservation_id))
            except Exception:
                sleep(2)
                continue
            last_state = res.state
            last_assigned = len(res.assigned_machines)
            if res.state == "R" and last_assigned >= request_count:
                self._hostnames = res.assigned_machines
                return
            sleep(2)

        # Use whatever we got — run() validates downstream and raises if short.
        try:
            res = self.preserve_manager.fetch_reservation(str(self.reservation_id))
            self._hostnames = res.assigned_machines
        except Exception:
            self._hostnames = []
        Print.warn(
            f"Reservation {self.reservation_id} not fully assigned after 60s "
            f"(state={last_state}, assigned={last_assigned}/{request_count})"
        )
    ...
    def _parse_hotstuff_logs(self, client_logs):
        ...
        if proc.returncode != 0:
            ...
            raise BenchError("process.py failed", RuntimeError(proc.stderr))
    ...
    def run(self, debug=False, local=True, flavor="themis"):
        ...
        try:
            ...
            if(
                ...
                if len(all_hostnames) < self._num_machines:
                    raise RuntimeError(
                        f"Reservation under-fulfilled: need {self._num_machines} usable "
                        f"machines after removing banned nodes, got {len(all_hostnames)}"
                    )
            ...
            # Silent-fault induction: now that all N replicas have come up
            # and bound their listeners, hard-kill the trailing F so clients
            # see established peers drop. Neither pompe-app nor the dikaios
            # hotstuff-app honors a --num-faults flag, so this kill loop is
            # the only place silent faults are actually injected.
            self._silence_last_f_replicas(num_faults, replica_IPs)
